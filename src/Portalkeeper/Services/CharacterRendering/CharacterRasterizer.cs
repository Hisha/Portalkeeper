...
internal static class CharacterRasterizer
{
    public static byte[] Render(string clientFolder,ArmoryCharacter character,CancellationToken token)
    {
        ...
        float angle=20*MathF.PI/180;var right=new Vector3(MathF.Sin(angle),-MathF.Cos(angle),0);var forward=new Vector3(MathF.Cos(angle),MathF.Sin(angle),0);
        ...
        var light=Vector3.Normalize(new Vector3(.7f,-.4f,.8f));
        foreach(var mesh in meshes)
        {
            var xy=new Vector2[mesh.Positions.Length];var z=new float[xy.Length];var lighting=new float[xy.Length];
            for(int i=0;i<xy.Length;i++)
            {
                ...
                lighting[i]=.55f+.45f*MathF.Max(Vector3.Dot(mesh.Normals[i],light),0);
            }
            foreach(var surface in mesh.Surfaces)
            {
                ...
                for(int t=0;t<surface.Indices.Length;t+=3)
                {
                    ...
                    for(int y=top;y<=bottom;y++)for(int x=left;x<=rightEdge;x++)
                    {
                        ...
                        float zz=w0*z[i0]+w1*z[i1]+w2*z[i2];int pixel=y*width+x;if(zz<=depth[pixel])continue;
                        ...
                        if(tex.Pixels[source+3]<128)continue;
                        float l=w0*lighting[i0]+w1*lighting[i1]+w2*lighting[i2];
                        for(int k=0;k<3;k++)output[pixel*4+k]=(byte)Math.Clamp((int)(tex.Pixels[source+k]*l),0,255);
                        output[pixel*4+3]=255;depth[pixel]=zz;
                    }
                }
            }
        }
