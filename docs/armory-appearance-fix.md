...
The fix is part of the application's existing rendering path. No additional renderer errors occurred for the two reported profiles. Client archives and published JSON were only read. The original repository was not edited; this ZIP contains complete source files copied from its current contents, preserving its existing changes.
